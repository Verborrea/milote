...
let lotesStyle = {polygonOptions: {fillColor: 'rgba(0, 255, 0, 0.25)', strokeColor: 'rgba(0, 0, 255, 0.5)',strokeThickness:0.1}};
...
function GetMap(){
  alert('ALERTA\n\n\nPor mientras, solo se cargará un cuadrado de lotes encima del aeropuerto');
  ...
  map = new Microsoft.Maps.Map("#map", {
    ...
    center: new Microsoft.Maps.Location(-16.329694, -71.567992),
    zoom: 15,
  });
        ...
        for(let i = 0; i < lenc; i++){
          if (lLayers[cargar[i]] === undefined){
            ...
            Microsoft.Maps.Events.addHandler(lLayers[cargar[i]], 'click', (e) => 
            {
              ...
              dir_via = lote_sel.metadata.title;
...
function colorear(layer,opacity){
  ...
  let color, scolor;
  for(let i = 0; i < lotesl.length; i++) coco(lotesl[i],opacity);
}
